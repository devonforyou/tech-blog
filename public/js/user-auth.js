...
async function handleSignUpClick(e) {
    ...
    if (response.ok) {
        ...
        document.location.replace(`/dashboard/${userData.username}`);
...
async function handleLogInClick(e) {
    ...
    const userData = {
        username: $("#signin-username").val().trim().toLowerCase(),
        password: $("#signin-password").val().trim(),
    };

    if (userData.username.length < 5) {
        showErrorMessage($('#signin-username'), '.login-short-username');
        return;
